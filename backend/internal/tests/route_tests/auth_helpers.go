...
func ensurePermissionForRole(t *testing.T, db *gorm.DB, roleID uuid.UUID, action models.PermissionAction, resource string) {
	...
	if err := db.Clauses(clause.OnConflict{DoNothing: true}).Create(&permission).Error; err != nil {
		t.Fatalf("failed to ensure permission %s %s: %v", action, resource, err)
	}

	if err := db.Where("action = ? AND resource = ?", action, resource).First(&permission).Error; err != nil {
		t.Fatalf("failed to fetch permission %s %s: %v", action, resource, err)
	}
	...
	if err := db.Clauses(clause.OnConflict{DoNothing: true}).Create(&rolePermission).Error; err != nil {
